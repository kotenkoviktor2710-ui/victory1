 ...
 */
function applyTitleGlitch(title, intensity) {
  if (!title) return;

  if (intensity <= 0.02) {
    title.classList.remove("main-menu__title--glitch-ramp");
    title.style.removeProperty("--title-glitch-opacity");
    title.style.removeProperty("--title-glitch-shift");
    return;
  }

  title.classList.add("main-menu__title--glitch-ramp");
  title.style.setProperty("--title-glitch-opacity", String(Math.min(1, intensity * 1.05)));
  title.style.setProperty("--title-glitch-shift", `${intensity * 11}px`);
}
 ...
 * @param {HTMLElement|null} params.title
 */
function startNoiseLoop({ token, stack, noiseCanvas, scanlines, title }) {
  ...
  const draw = (now) => {
    ...
    applyTitleGlitch(title, burstIntensity);
...
export function startMenuBgCycle(root) {
  ...
  const title = root.querySelector(".main-menu__title");
  ...
  if (!reducedMotion) {
    startNoiseLoop({ token, stack, noiseCanvas, scanlines, title });
  }
