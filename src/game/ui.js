...
import { t, getItemSearchPattern, getDefaultItemLabel, getUiValue } from "./i18n.js";

export function getGameTitle() {
  return t("game.title");
}

export function getGameTitleLines() {
  const lines = getUiValue("game.titleLines");
...
export function isItemSearchLine(line) {
  return line?.kind === "narration" && getItemSearchPattern().test(String(line.text ?? ""));
}
...
export function getItemSearchItems(line) {
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      id: `item-${index + 1}`,
      label: labels[index] ?? getDefaultItemLabel(index + 1),
      x: position.x,
      y: position.y,
      size: position.size,
 ...
 * @param {{ music: number, sfx: number, sync: number, scanner: number }} options.settings
 */
export function renderMainMenu({ hasSave, menuOverlay, discovery, storyFlowchartLayout, settings }) {
  ...
  const titleLines = getGameTitleLines();
  ...
  return `
    <div class="screen screen--menu main-menu">
      ...
      <div class="main-menu__center">
        <div class="main-menu__title-wrap">
          <h1 class="main-menu__title">
            <span class="main-menu__title-line is-glitching" data-text="${escapeAttr(titleLines[0])}">${escapeHtml(titleLines[0])}</span>
            <span class="main-menu__title-line is-glitching" data-text="${escapeAttr(titleLines[1])}">${escapeHtml(titleLines[1])}</span>
          </h1>
        </div>

        <div class="main-menu__nav-wrap">
          <nav class="main-menu__nav" aria-label="${escapeAttr(t("menu.navLabel"))}">
            ${menuButton("new-game", t("menu.newGame"), { active: true })}
            ${menuButton("continue", t("menu.continue"), { disabled: !hasSave })}
            ${menuButton("story", t("menu.story"))}
          </nav>
        </div>
      </div>
      ...
      <button type="button" class="main-menu__settings" data-action="settings-quick" aria-label="${escapeAttr(t("menu.settings"))}">
...
function renderScanHelpModal() {
  const steps = getUiValue("gameplay.scanHelpSteps");
    ...
    <div class="overlay overlay--dim" data-stop-advance="true">
      <div class="panel panel--scan-help">
        <div class="panel__head">
          <h2 class="panel__title">${escapeHtml(t("gameplay.scanHelpTitle"))}</h2>
          <button type="button" class="panel__close" data-action="close-scan-help" aria-label="${escapeAttr(t("menu.close"))}">
        ...
        <div class="panel__foot">
          ${menuButton("close-scan-help", t("gameplay.understood"), { active: true, block: true })}
        </div>
      </div>
    </div>
  `;
    ...
    <div class="${getNovelClassName(showActBreak, showChoices, vm.line)}" data-action="advance">
      ...
      <header class="novel-hud">
        <div class="novel-hud__actions">
          <button type="button" class="hud-btn hud-btn--menu" data-action="game-menu" aria-label="${escapeAttr(t("menu.menuTitle"))}">
...
export function renderEndScreen(options = {}) {
  const title = options.title ?? t("end.title");
  ...
  return `
    <div class="screen screen--end end-screen">
      ...
      <div class="end-screen__content">
        <p class="end-screen__kicker">${escapeHtml(getGameTitle())}</p>
        <h1 class="end-screen__title">${escapeHtml(title)}</h1>
        <p class="end-screen__note">${escapeHtml(t("end.note"))}</p>

        <div class="end-screen__actions">
          ${menuButton("exit", t("menu.toMainMenu"), { active: true, block: true })}
        </div>
      </div>
    </div>
  `;
...
/** @param {{ music: number, sfx: number, sync: number, scanner: number }} settings @param {{ inGame?: boolean }} [options] */
  ...
  return `
    <div class="overlay" data-stop-advance="true">
      <div class="panel panel--settings">
        <div class="panel__head">
          <h2 class="panel__title">${renderIcon("settings", "icon panel__title-icon")} ${escapeHtml(t("settings.title"))}</h2>
          <button type="button" class="panel__close" data-action="${closeAction}" aria-label="${escapeAttr(t("menu.close"))}">
            ${renderIcon("close", "icon panel__close-icon")}
          </button>
        </div>

        <div class="panel__body">
          ${settingsRow("sync", t("settings.sync"), settings.sync, "volumeHigh")}
          ${settingsRow("sfx", t("settings.sfx"), settings.sfx, "volumeHigh")}
          ${settingsRow("music", t("settings.music"), settings.music, "volumeHigh")}
          ${settingsRow("scanner", t("settings.scanner"), settings.scanner, "volumeHigh")}
        </div>

        <div class="panel__foot">
          ${menuButton(closeAction, t("menu.close"), { active: true, block: true })}
        </div>
      </div>
    </div>
  `;
    ...
    return `
      <div class="story-flowchart" data-stop-advance="true">
        ...
        <header class="story-flowchart__head">
          <button type="button" class="hud-btn story-flowchart__close" data-action="close-overlay" aria-label="${escapeAttr(t("menu.close"))}">
            ${renderIcon("close", "icon hud-btn__icon")}
          </button>
          <div class="story-flowchart__intro">
            <h2 class="story-flowchart__title">${escapeHtml(getGameTitle())}</h2>
          </div>
        </header>

        <div class="story-flowchart__viewport" data-flowchart-viewport>
          <p class="story-flowchart__loading">${escapeHtml(t("flowchart.loading"))}</p>
        </div>
      </div>
    `;
  }
...
function renderGameMenuModal(progress) {
  const safeProgress = progress ?? {
    ...
    total: 1,
    percent: 0,
    currentTitle: t("progress.prologue"),
  };
  ...
  return `
    <div class="overlay overlay--left" data-stop-advance="true">
      <div class="panel panel--game-menu">
        <div class="panel__head">
          <h2 class="panel__title">${escapeHtml(t("menu.menuTitle"))}</h2>
          <button type="button" class="panel__close" data-action="close-game-overlay" aria-label="${escapeAttr(t("menu.close"))}">
            ${renderIcon("close", "icon panel__close-icon")}
          </button>
        </div>

        <div class="panel__body panel__body--stack">
          ${gameMenuItem("exit", t("menu.mainMenu"))}
          ${gameMenuItem("game-settings", t("menu.settings"))}
...
function renderGameProgressBlock(progress) {
  return `
    <section class="game-progress" aria-label="${escapeAttr(t("progress.sectionLabel"))}">
      <div class="game-progress__head">
        <div class="game-progress__titles">
          <h3 class="game-progress__title">${escapeHtml(t("progress.title"))}</h3>
          <p class="game-progress__subtitle">${escapeHtml(t("progress.subtitle"))}</p>
        </div>
      </div>
      ...
      <div
        ...
        aria-label="${escapeAttr(t("progress.percentLabel", { percent: progress.percent }))}"
      >
        <span class="game-progress__bar-fill" style="width: ${progress.percent}%"></span>
      </div>

      <p class="game-progress__meta">${escapeHtml(
        t("progress.meta", {
      ...
      <p class="game-progress__current">${escapeHtml(
        t("progress.current", { title: progress.currentTitle }),
      )}</p>
    </section>
  `;
}
    ...
    <div class="act-break">
      <div class="act-break__card">
        ...
        <div class="act-break__hint">
          <span class="act-break__hint-text">${escapeHtml(t("gameplay.actBreakHint"))}</span>
...
function renderScanTerminalPanel(line, searchState) {
  ...
  return `
    <section
      ...
      aria-label="${escapeAttr(t("gameplay.scanAria"))}"
      ${isComplete ? 'data-scan-complete="true"' : ""}
      data-stop-advance="true"
    >
      <div class="textbox__body scan-terminal__body">
        <div class="textbox__name scan-terminal__name">${escapeHtml(t("gameplay.scanTitle"))}</div>
        ${renderChoicePlaqueList(plaques, { ariaLabel: t("gameplay.scanLogAria") })}
      </div>
    </section>
  `;
}
  ...
  return `
    <div class="anomaly-scan is-scanning" aria-hidden="true" style="--scan-y: 0%;">
      ...
      <button
        ...
        data-stop-advance="true"
        aria-label="${escapeAttr(t("gameplay.scanHelpAria"))}"
      >?</button>
    </div>
  `;
}
